...
// ==== 業種 別 の hero コピー (v2 · 45社 深掘り + 「刺さる」 pattern 反映) ====
// SSOT: memory `reference_hp_catchcopy_business_types.md` + `research_hp_catchcopy_deepdive_2026-08-17.md`
// 5 archetype: A. 感情 / B. 数字proof / C. 詩的 / D. 逆説 / E. 語りかけ
// D (逆説) 中心 の 選定: 「業界 の 王道 を 明示 否定」 が テンプレ感 完全 排除 · owner 「まだ甘い」対応
function heroCopyForCategory(cat, area) {
  const areaTxt = area || '地域';
  if (!cat) return {
    // 12-D 逆説: 「駅から遠い、看板も小さい」
    eyebrow: `${areaTxt} · Local`,
    h1line: `駅から遠い、<em>看板も</em>小さい。`,
    sub: 'それでも来てくれる客のために、中の時間だけ、丁寧に作っています。'
  };

  // ネイル (2-A 感情: 「指先が変わると、目線が変わる」)
  // 順序注意: ネイル → 美容 (「ネイルサロン」→HAIR 誤hit 防止)
  if (/ネイル/.test(cat)) return {
    eyebrow: `${areaTxt} · ネイルサロン`,
    h1line: `指先が変わると、<em>目線が</em>変わる。`,
    sub: 'ハンドケア15分、アート40分。忙しい日常のなかで、ふと手元を見たときに、気分が一段上になるデザインを。'
  };

  // まつげ (3-A 感情: 「朝、鏡の前の5分を、返します」)
  if (/まつげ|まつ毛|アイラッシュ/.test(cat)) return {
    eyebrow: `${areaTxt} · アイラッシュ`,
    h1line: `朝、鏡の前の<em>5分を</em>、返します。`,
    sub: 'メイク時間が短くなる。目が一段大きく見える。40日に一度の来店で、毎朝の気持ちが少し変わります。'
  };

  // 美容室 (1-D 逆説: 「派手なスタイル、うちにはありません」)
  if (/美容|サロン|ヘア/.test(cat)) return {
    eyebrow: 'トレンド、追いません。',
    h1line: `派手なスタイル、<em>うちには</em>ありません。`,
    sub: '毎日セットするのは、あなた自身。だから「朝5分で決まる髪」を、誰より真剣に作ります。'
  };

  // エステ·リラクゼーション (4-D 逆説: 「一度の施術で変わる、とは約束しません」)
  if (/エステ|マッサージ|リラク|フェイシャル/.test(cat)) return {
    eyebrow: '「今すぐ痩せる」、言いません。',
    h1line: `一度の施術で変わる、<em>とは</em>約束しません。`,
    sub: '半年通って、鏡で気づくくらいが一番良いと考えています。だから初回で6ヶ月分の計画を一緒に決めます。'
  };

  // カフェ (5-D 逆説 · star pick: 「Wi-Fiありません。仕事は、ここではしないでください」)
  if (/カフェ|喫茶|cafe/i.test(cat)) return {
    eyebrow: 'Wi-Fi、ありません。',
    h1line: `仕事は、ここでは<em>しないで</em>ください。`,
    sub: 'パソコンを開かず、コーヒーと向き合う15分を買いに来てほしい店。朝9時から焙煎、11時にドリップ。'
  };

  // 寿司·和食·高級飲食 (6-D 逆説: 「メニュー、置いていません」)
  if (/寿司|鮨|割烹|料亭|懐石|和食|日本料理/.test(cat)) return {
    eyebrow: 'ネタ、選べません。',
    h1line: `メニュー、<em>置いて</em>いません。`,
    sub: '大将が決めます。「これが今日のうち」を味わってもらう店。それが嫌な方は、他店をおすすめします。'
  };

  // ホステル·旅館 (7-D 逆説 · star pick: 「観光地には、行かないでください」)
  if (/ホステル|旅館|民宿|民泊|温泉|宿/.test(cat)) return {
    eyebrow: '観光、そんなにしなくていい。',
    h1line: `観光地には、<em>行かないで</em>ください。`,
    sub: `この街の「昼下がりの商店街」「夜の銭湯」が、実は一番の記憶に残ります。その案内だけ、こちらで差し上げます。`
  };

  // ジム·フィットネス (8-D 逆説 · star pick: 「『三ヶ月で-10kg』、うちにはありません」)
  if (/ジム|フィットネス|パーソナル|ヨガ|ピラティス/.test(cat)) return {
    eyebrow: '短期集中、やりません。',
    h1line: `「三ヶ月で-10kg」、<em>うちには</em>ありません。`,
    sub: 'それ、リバウンドするから。「一年後もその体重」を前提に、週2回のゆるい継続を提案します。'
  };

  // イタリアン·フレンチ (9-D 逆説: 「ドレスコード、ありません」)
  if (/イタリアン|フレンチ|ビストロ|レストラン/.test(cat)) return {
    eyebrow: '服装、自由。',
    h1line: `ドレスコード、<em>ありません</em>。`,
    sub: '「気軽な一夜」を、皿だけ本気で作る店。スニーカーでも、緊張せずに来てください。'
  };

  // ラーメン·大衆 (10-D 逆説 · star pick: 「『濃厚』『豚マシマシ』、うちにはありません」)
  if (/ラーメン|そば|うどん|中華|食堂/.test(cat)) return {
    eyebrow: '二郎系、ではない。',
    h1line: `「濃厚」「豚マシマシ」、<em>うちには</em>ありません。`,
    sub: '淡口醤油と出汁の一杯。「食後に、まだ何か食べられる」くらいの軽さを、看板にしています。'
  };

  // 歯科·デンタル (11-A 感情 · star pick: 「10年後も、自分の歯で笑うために」)
  if (/歯科|デンタル|クリニック|医院|病院|dental|clinic/i.test(cat)) return {
    eyebrow: `${areaTxt} · 予防歯科`,
    h1line: `10年後も、<em>自分の歯で</em>笑うために。`,
    sub: '治療ではなく、予防。半年に一度の定期検診を、30分の短時間で続けやすく設計しています。'
  };

  // fallback (12-D 逆説: 「駅から遠い、看板も小さい」)
  return {
    eyebrow: '便利、ではありません。',
    h1line: `駅から遠い、<em>看板も</em>小さい。`,
    sub: 'それでも来てくれる客のために、中の時間だけ、丁寧に作っています。'
