...
function heroCopyForCategory(cat, area) {
  ...
  // エステ·リラクゼーション (Plan B: 「効果否定」 D は 本業自殺 → v0.9.0 良い h1 に 戻す)
  // eyebrow は D 逆説 「今すぐ痩せる」で 差別化、 h1 sub は 感情 A 型 で 効果 訴求 維持
  ...
  // カフェ (Plan B refine: eyebrow だけ D 逆説 「Wi-Fi ありません」· h1 は A 感情 「一日の中の、静かな15分」で 二重拒絶 回避)
  if (/カフェ|喫茶|cafe/i.test(cat)) return {
    eyebrow: 'Wi-Fi、ありません。',
    h1line: `一日の中の、<em>静かな</em>15分に。`,
    sub: '朝9時から焙煎、11時にドリップ。その日の温度と湿度で抽出を変える、手で淹れる一杯。'
  };

  // 寿司·和食 (reviewer 46/50 提案: eyebrow だけ D 「ネタ、選べません」· h1 は C 詩的 「その日の海と、向き合う」)
  // 「除去 (eyebrow) → 提供 (h1)」 の 分業 で 二重拒絶 回避
  ...
  // イタリアン·フレンチ (Plan B: 「服装問題」 D は fine dining 格 を 矮小化 → terroir 型 に 戻す)
  // eyebrow だけ D 「服装、自由」 で 敷居下げ 差別化、 h1 は terroir 詩的 で 格 維持
  ...
  // ラーメン·大衆 (Plan B: 「二郎系否定」 は 二郎系 店 では 逆効果 → 「仕込み時間」職人証言 に 戻す · v0.9.1 43点 pattern)
  ...
  // fallback (Plan B: 「駅から遠い」D は 業種不明 全店 に 押付け 汎用性ゼロ → 汎用 「居場所」型 に 戻す)
  return {
    eyebrow: `${areaTxt} · ${cat}`,
    h1line: `${areaTxt}に、<em>居場所を</em>ひとつ。`,
    sub: '派手さはなくても、通うたび少しだけ気分が上がる。そんなお店を、この街で続けています。'
