...
(function () {
  ...
  const VALID_VIEWS = ['dashboard','clients','timeline','meetingHistory','lineChat','leadHub','distributionHub','birthdayTab','calendarTab','settingsHub','dormantFollowup','tagsHub','kpi'];
  function activateTab(name, options) {
    ...
    if (name === 'lineChat') {
      if (window.LineApp && window.LineApp.renderLineChatHub) {
        if (!window._lineInited) { window.LineApp.init(); window._lineInited = true; }
        window.LineApp.renderLineChatHub();
      } else {
        window._pendingLineView = 'lineChat';
      }
    }
...
(function () {
  ...
  function setPending(arr) {
    try { localStorage.setItem(STORAGE, JSON.stringify(arr)); } catch (_) {}
  }
  function prunePending(arr) {
    const now = Date.now();
    return arr.filter(p =>
      (now - p.startedAt) < 24 * 3600e3 &&
      (p.status !== 'completed' || (now - (p.completedAt || 0)) < 30 * 60e3)
    );
  }
  ...
  window.renderZoomPendingPill = function () {
    ...
    setPending(list);
    let container = document.getElementById('fp-pending-zoom-panel');
    if (list.length === 0) {
