 ...
 *
 * - 前日 18:00 JST → 翌日チェックイン予定のお客様に道順 Flex push
 * - 当日 07:00 JST → 本日チェックイン予定のお客様に鍵案内 Flex push
 *
 * 二重送信防止: reservations.remindedPre / remindedArrival フラグで gate
 */

const functions = require('firebase-functions/v2/scheduler');
const admin = require('firebase-admin');
const { Client } = require('@line/bot-sdk');
const reminderFlex = require('../webhook/templates/reminder');
const arrivalFlex = require('../webhook/templates/arrival');
...
exports.preCheckinReminder = functions.onSchedule({
  ...
  for (const docSnap of snap.docs) {
    ...
    try {
      const [bldg, room] = await Promise.all([
        fetchBuilding(rec.buildingId),
        fetchRoom(rec.roomId),
      ]);
